...
//        }
    ...
    */
    ...
    private void TurnManager() {
        if(senko == true) { //自分が先攻なら
            if(turn == 1) { //1ターン目
                x_my = (int)(Math.random() * Field.SIZE); //自分の座標を決める
                y_my = (int)(Math.random() * Field.SIZE);
                RyodoMark(true); //自分の領土にマーク
            }
            else { //2~16ターン目
                enemyPowerSearch();
                RyodoMark(false); //相手の領土分布にマークする
                myBrain(); //領土分布と自分の武器を基に、自分が爆撃する(X,Y)座標を決めるメソッド
                RyodoMark(true); //自分の領土分布にマークする
            }
        }
        else { //自分が後攻なら
            enemyPowerSearch();
            RyodoMark(false); //相手の領土にマークする
            myBrain();
            RyodoMark(true); //自分の領土にマークする
        }
    }
    ...
    */
    protected void action(robobattler2.Missile missile) {
        ...
        TurnManager(); //先攻後攻によって処理を切り替えるメソッド
