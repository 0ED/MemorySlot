...
public class BTree
{
    ...
    private class InternalNode extends Node {
        int           nChilds;  // Ç±ÇÃêﬂÇ™Ç‡Ç¡ÇƒÇ¢ÇÈéqÇÃêî
        Node[]        child;    // ïîï™ñÿ
        Integer[]     low;      // äeïîï™ñÿÇÃç≈è¨ÇÃóvëf

        private InternalNode() {
            serial = serialNumber++;    // ÉVÉäÉAÉãî‘çÜÇïtÇØÇÈ
            nChilds = 0;
    ...
    final private static int  MAX_CHILD = 5;    // å‹ï™ñÿ
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        while ((str = input.readLine()) != null) {
            ...
            if (command == 'q') {
                ...
                if (tree.setData(arg)) {
                    System.out.println("íl" + arg + "ÇÃê›íËÇ…ê¨å˜ÇµÇ‹ÇµÇΩÅB");
                } else {
                    System.out.println("íl" + arg + "ÇÃê›íËÇ…é∏îsÇµÇ‹ÇµÇΩÅB");
                }
                ...
                try {
                    ...
                    System.err.println("êÆêîà»äOÇÃÇ‡ÇÃÇ™éwíËÇ≥ÇÍÇ‹ÇµÇΩ:" + arg);
                    continue;
                }

                if (command == '+') {
                    if (tree.insert(num, "[" + num + "]")) {
                        System.out.println(num + "ÇÃë}ì¸Ç…ê¨å˜ÇµÇ‹ÇµÇΩÅB");
                    } else {
                        System.out.println(num + "ÇÃë}ì¸Ç…é∏îsÇµÇ‹ÇµÇΩÅB");
                    }
                } else if (command == '-') {
                    if (tree.delete(num)) {
                        System.out.println(num + "ÇÃçÌèúÇ…ê¨å˜ÇµÇ‹ÇµÇΩÅB");
                    } else {
                        System.out.println(num + "ÇÃçÌèúÇ…é∏îsÇµÇ‹ÇµÇΩÅB");
                    }
                } else if (command == '/') {
                    if (tree.search(num)) {
                        System.out.println(num + "Ç™å©Ç¬Ç©ÇËÇ‹ÇµÇΩÅBíl=" + tree.getData());
                    } else {
                        System.out.println(num + "Ç™å©Ç¬Ç©ÇËÇ‹ÇπÇÒÇ≈ÇµÇΩÅB");
                    }
                }
            } else {
                System.out.println("ÉRÉ}ÉìÉh" + command +
                                   "ÇÕÉTÉ|Å[ÉgÇ≥ÇÍÇƒÇ¢Ç‹ÇπÇÒÅB");
            }
            System.out.print(">");
        }
    }
